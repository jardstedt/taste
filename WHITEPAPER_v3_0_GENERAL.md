...
The agent-to-agent economy is real. On Virtuals Protocol alone — the largest on-chain agent commerce network — over 10,600 AI agents are registered, with $300M+ in cumulative agent-to-agent transactions and 1.77 million completed jobs. These agents buy and sell services from each other using an open marketplace protocol called ACP (Agent Commerce Protocol), where any agent can list offerings and any other agent can purchase them, with payments settled automatically in USDC stablecoins on the Base blockchain. Similar agent commerce networks are emerging across the industry.

This is not a niche phenomenon. Google, Microsoft, OpenAI, and Anthropic are all independently building infrastructure for an agent economy. Google launched the Agent2Agent protocol (150+ partner organizations, now under the Linux Foundation) and an Agent Payments Protocol for autonomous commerce. Microsoft published "The Agentic Economy" — a research paper calling for open agent marketplaces — and is investing $125B in supporting infrastructure. OpenAI and Anthropic have each released their own agent interoperability protocols. Every major technology company is converging on the same thesis: AI agents will transact autonomously at scale. All of them are building the roads. None of them are building the human quality layer those roads will need.
...
Agent commerce protocols include evaluator roles — a third party that assesses whether a delivery met the agreed terms. Today, every evaluator is an AI agent. AI evaluating AI. This works for objective checks: did the file render, was the response returned in time, does the data parse. It fails for anything subjective: was the creative brief actually followed, is the research conclusion sound, does the marketing video achieve its goal.

As the agent economy scales — more agents, higher-value transactions, more complex deliverables — the need for trustworthy arbitration grows proportionally. Taste is the only service offering human arbitration in this space. Every other evaluator is a machine checking another machine's homework.
...
**Dispute Arbitration**
Third-party human evaluation of a job delivery between two other agents. Expert reviews whether the provider fulfilled the original contract, assesses deliverable quality against requirements, submits an approve/reject verdict with structured reasoning. **The only human arbitration service in the agent economy.** Architecturally unique — Taste can serve as the evaluator for any agent-to-agent job, receiving the deliverable and routing it to a human expert for judgment.

**Content Quality Gate**
Pre-publish review of AI-generated content (video, images, audio, memes) for cultural sensitivity, derivative elements, brand safety, and audience appeal before distribution. Expert delivers a verdict with cultural sensitivity score, brand safety score, and flagged issues.

**Trust Evaluation**
Agent submits a project, entity, or opportunity for human due diligence. Expert assesses legitimacy, checks community authenticity, reviews claims, delivers a structured verdict with confidence score, key findings, red flags, and positive signals.

**Output Quality Gate**
AI-generated output (research reports, analysis, intelligence briefs) reviewed by a human for accuracy, coherence, and whether the conclusions follow from the evidence. Expert delivers a quality verdict with score, issues found, and suggested improvements. Addresses the core AI hallucination problem — a human catches what automated checks miss.
...
Taste can serve as the evaluator itself: when assigned as a third-party evaluator on another provider's job, it creates a dispute arbitration session where a human expert reviews the original deliverable and submits an approve/reject verdict. This means Taste operates as both a service provider and an evaluator within the same system.
...
Payments are settled in USDC (a digital dollar stablecoin, always worth $1) on the Base blockchain. The buyer's payment is held in escrow until the job is delivered and verified. Settlement is near-instant and near-free — a full payment cycle costs about $0.03 in network fees. This enables a transaction model that traditional payment rails cannot support: autonomous micropayments of $1–$5 between a machine and a human, globally, with no invoicing, no bank accounts, and no minimum transaction size.
...
```json
{
  ...
  "disclaimer": "This is a qualitative human opinion used for dispute resolution."
}
```

Assessment fields vary per offering type. Transcripts are included only when actual back-and-forth conversation occurred.
...
Despite $65M+ in adjacent funding and a landmark thesis from Multicoin Capital ("Inverting the Human-Agent Relationship"), no company has shipped a production-grade marketplace where AI agents pay humans for structured expert judgment.
...
| **Human API** | $65M funded. Agents coordinate humans for structured tasks. | Modernized Mechanical Turk. Fiat payments, no agent marketplace integration. |
...
No major AI lab — OpenAI, Anthropic, or Google — has shipped a dedicated agent-to-human expert product. They provide primitives, not products.
...
Reputation is domain-specific — excellence in one area does not inflate credibility in another. Repeat-buyer signals are the strongest quality indicator.
...
**For artists in emerging economies, this is particularly meaningful.** A session fee of a few dollars for 15 minutes of expert judgment represents a competitive hourly rate in much of the world — and the work is location-independent, requires no equipment beyond a phone, and pays in globally liquid currency within seconds. An illustrator in Lagos, a filmmaker in Manila, a musician in São Paulo can monetize their creative judgment for a global AI economy without relocating, without intermediaries, and without waiting 30 days for an invoice to clear. The barriers to entry are expertise and a phone. The barriers that usually lock out global South talent — geography, banking access, employer relationships — do not apply.

**Long term, we are building toward a world where AI and humans work together.** Not AI replacing humans. Not humans supervising AI. Genuine collaboration — where the expert and the agent reason through a creative problem together, each contributing what the other lacks. The message bridge is the first step. Bidirectional workspaces are next. The endgame is a new kind of creative partnership between human taste and machine capability.
...
**Transparency.** Every job completion and rejection is recorded on-chain. Our approval and rejection rates are publicly verifiable — not because we publish them, but because anyone can check.
...
Platform built and deployed: 8 live offerings, form-first expert workflow, marketplace integration with message bridge, push notifications, structured deliverables with per-offering schema validation, mobile-optimized expert dashboard (PWA), public resource endpoints, security-hardened for production.

**Phase 2 — First Revenue (Weeks 1–4 post-launch)**
Graduate from sandbox to live marketplace (the protocol requires agents to demonstrate functionality in a test environment before accessing real transactions). Validate thesis with real jobs and real revenue. Direct outreach to creative agent builders. Target: 50 completed jobs, 10 unique buyers.
...
**Evolution:** The interaction becomes bidirectional. As agents evolve to support conversational patterns (the message bridge is already built and waiting), sessions become real-time collaborations — the expert asks clarifying questions, the agent provides additional context, the assessment gets richer. Within a session, the expert can ask the AI to run calculations, search data, generate drafts — turning evaluation into genuine creative partnership.
